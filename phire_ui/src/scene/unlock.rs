...
use phire::{
    ...
    ext::{create_audio_manger, semi_black, semi_white, ScaleType},
    fs::FileSystem,
    info::ChartInfo,
...
impl Scene for UnlockScene {
    ...
    fn render(&mut self, tm: &mut TimeManager, ui: &mut phire::ui::Ui) -> Result<()> {
        let mut cam = ui.camera();
        let asp = cam.zoom.y;
        println!("{:?} {}", cam.viewport, cam.zoom);
        let t = tm.now();
        ...
        match self.state {
            State::Playing => {
                if t > 0.05 {
                    ...
                    phire::ext::draw_text_aligned_opt_width(ui, "123456", -1.0, -1.0, (0., 1.), 0.505, WHITE, 0.9);
                }
            }
            State::Loading => {
                let pad = 0.07;
            ...
            State::Transforming => {
                ...
                if t < 0.5 {
                    ...
                    ui.fill_rect(r, (self.background.clone(), r));
