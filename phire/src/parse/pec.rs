...
use crate::{
    core::{
        Anim, AnimFloat, AnimFloatF64, AnimVector, BpmList, Chart, ChartExtra, ChartSettings, JudgeLine, JudgeLineCache, JudgeLineKind, Keyframe, Note, NoteKind, Object, TweenId, EPS
    },
    ext::NotNanExt,
...
fn parse_speed_events(mut pec: Vec<(f32, f64)>, max_time: f32) -> AnimFloatF64 {
    if pec[0].0 >= EPS {
        pec.insert(0, (0., 0.));
    }
    let mut kfs = Vec::new();
